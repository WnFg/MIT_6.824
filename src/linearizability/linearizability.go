...
func report(entry *node) {
	head := entry
	for head != nil {
		//fmt.Printf("all entry: %v\n", head)
		head = head.next
	}
	//fmt.Printf("\n\n")
}
...
func checkSingle(model Model, subhistory *node, kill *int32) bool {
	...
	for headEntry.next != nil {
		...
		if entry.match != nil {
			...
			//	fmt.Printf("ok:%v, old_state:%v, new_state:%v, input:%v, output:%v\n",
			//		ok, state, newState, entry.value, matching.value)
			//	fmt.Printf("match Entry: %v\n", entry)
			if ok {
